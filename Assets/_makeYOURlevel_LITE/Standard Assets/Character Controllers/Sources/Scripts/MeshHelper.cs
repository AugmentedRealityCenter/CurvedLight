...
public static class MeshHelper
{
	...
	public static void Subdivide4(Mesh mesh)
	{
		...
		InitArrays(mesh);
		int[][] triangless = new int[mesh.subMeshCount][];
		List<int>[] indicess = new List<int>[mesh.subMeshCount];
		for (int sm=0; sm < mesh.subMeshCount; sm++) {
			triangless[sm] = mesh.GetTriangles (sm);
			indicess[sm] = new List<int>();
			for (int i = 0; i < triangless[sm].Length; i += 3) {
				int i1 = triangless[sm] [i + 0];
				int i2 = triangless[sm] [i + 1];
				int i3 = triangless[sm] [i + 2];
				...
				indicess[sm].Add (a);
				indicess[sm].Add (b);
				indicess[sm].Add (c); // center triangle
		}
	}
		...
		for (int sm=0; sm<mesh.subMeshCount; sm++) {
						mesh.SetTriangles (indicess [sm].ToArray (), sm);
				}
